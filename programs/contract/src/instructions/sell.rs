...
use super::{get_phoenix_best_price, get_openbook_best_price};

pub fn handler_sell<'info>(
    ctx: &Context<'_, '_, '_, 'info, SmartSell<'info>>,
    base_amount: u64 // Ex: USDC,USDT,..
) -> Result<()> {
    ...
    // Sell at OpenBook
    if phoenix_best_bid_price <= openbook_best_bid_price {
        ...
        invoke_signed(&
            openbook_dex::instruction::new_order(
                ...
                ctx.accounts.rent_sysvar_id.key,
                Option::None,
                ctx.accounts.openbook_program_id.key,
                openbook_dex::matching::Side::Ask,
                NonZeroU64::new(openbook_best_bid_price/openbook_market.pc_lot_size).unwrap(),
                NonZeroU64::new(base_amount).unwrap(),
                openbook_dex::matching::OrderType::ImmediateOrCancel,
                0,
                openbook_dex::instruction::SelfTradeBehavior::DecrementTake,
                u16::MAX,
                NonZeroU64::new(base_amount*openbook_best_bid_price/10u64.pow(ctx.accounts.base_mint.decimals.into())).unwrap(),
                i64::MAX,
        ...
        invoke_signed(
            &phoenix::program::create_new_order_instruction_with_custom_token_accounts(
                ...
                &phoenix::state::OrderPacket::ImmediateOrCancel {
                    side: phoenix::state::Side::Ask,
                    num_base_lots: phoenix::quantities::BaseLots::new(base_amount),
                    num_quote_lots: phoenix::quantities::QuoteLots::new(0),
    ...
    // Retrieve the balances of the tokens after making a sell.
    ...
    if quote_token_balance_after > quote_token_balance_before {
        let sold_amount = base_token_balance_before - base_token_balance_after;
        let quote_amount = quote_token_balance_after - quote_token_balance_before;
        let sold_price = quote_amount * 10u64.pow(ctx.accounts.base_mint.decimals.into()) / sold_amount;

        // Write in JSON format for future parsing
        sol_log(&format!(r#"SOLD:{{"amount":{},"spend":{},"price":{}}}"#, sold_amount, quote_amount, sold_price));
    }
    else {
        msg!("BUY:Fail")
    }
